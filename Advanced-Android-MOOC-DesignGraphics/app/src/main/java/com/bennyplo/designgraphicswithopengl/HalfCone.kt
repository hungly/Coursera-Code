...
class HalfCone {
    ...
    init {
        ...
        val bb = ByteBuffer.allocateDirect(
            halfConeVertices.size * Float.SIZE_BYTES
        ) // (# of coordinate values * 4 bytes per float)
        bb.order(ByteOrder.nativeOrder())
        vertexBuffer = bb.asFloatBuffer()
        vertexBuffer.put(halfConeVertices)
        vertexBuffer.position(0)
        vertexCount = halfConeVertices.size / COORDS_PER_VERTEX
        val cb = ByteBuffer.allocateDirect(
            halfConeColors.size * Float.SIZE_BYTES
        ) // (# of coordinate values * 4 bytes per float)
        cb.order(ByteOrder.nativeOrder())
        colorBuffer = cb.asFloatBuffer()
        colorBuffer.put(halfConeColors)
        colorBuffer.position(0)
        val ib = IntBuffer.allocate(halfConeIndexes.size)
        indexBuffer = ib
        indexBuffer.put(halfConeIndexes)
