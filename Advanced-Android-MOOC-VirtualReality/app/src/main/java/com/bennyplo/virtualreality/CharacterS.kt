...
class CharacterS {
    ...
    private fun createCurve(controlPtsRight: FloatArray, controlPtsLeft: FloatArray) {
        ...
        for (segment in 0 until noSegments) {
            t = 0.0
            while (t < 1.0) {
                x = (1.0 - t).pow(3.0) * controlPtsRight[controlPtIndex] + controlPtsRight[controlPtIndex + 2] * 3 * t * (1 - t).pow(
                    2.0
                ) + controlPtsRight[controlPtIndex + 4] * 3 * t * t * (1 - t) + controlPtsRight[controlPtIndex + 6] * t.pow(
                    3.0
                )
                y = (1.0 - t).pow(3.0) * controlPtsRight[controlPtIndex + 1] + controlPtsRight[controlPtIndex + 3] * 3 * t * (1 - t).pow(
                    2.0
                ) + controlPtsRight[controlPtIndex + 5] * 3 * t * t * (1 - t) + controlPtsRight[controlPtIndex + 7] * t.pow(
                    3.0
                )
                xl = (1.0 - t).pow(3.0) * controlPtsLeft[controlPtIndex] + controlPtsLeft[controlPtIndex + 2] * 3 * t * (1 - t).pow(
                    2.0
                ) + controlPtsLeft[controlPtIndex + 4] * 3 * t * t * (1 - t) + controlPtsLeft[controlPtIndex + 6] * t.pow(
                    3.0
                )
                yl = (1.0 - t).pow(3.0) * controlPtsLeft[controlPtIndex + 1] + controlPtsLeft[controlPtIndex + 3] * 3 * t * (1 - t).pow(
                    2.0
