...
class CharacterA {
    ...
    init {
        ...
        val bb =
            ByteBuffer.allocateDirect(CharAVertex.size * Float.SIZE_BYTES) // (# of coordinate values * 4 bytes per float)
        bb.order(ByteOrder.nativeOrder())
        vertexBuffer = bb.asFloatBuffer()
        vertexBuffer.put(CharAVertex)
        vertexBuffer.position(0)
        val cb =
            ByteBuffer.allocateDirect(CharAColor.size * Float.SIZE_BYTES) // (# of coordinate values * 4 bytes per float)
