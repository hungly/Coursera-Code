...
class FlatSurfaceMyChar(private val context: Context?) {
...
//                "   gl_FragColor = vec4(vColor.xyz * vAmbientColor, 1) + specularColor + diffuseColor;" +
    ...
    init {
        ...
        val nb1 = ByteBuffer.allocateDirect(FlatSurfaceNormals.size * 4)
        nb1.order(ByteOrder.nativeOrder())
        normalBuffer = nb1.asFloatBuffer()
        normalBuffer.put(FlatSurfaceNormals)
        normalBuffer.position(0)

        // initialize vertex byte buffer for shape coordinates
        val bb =
            ByteBuffer.allocateDirect(FlatSurfaceVertexes.size * 4) // (# of coordinate values * 4 bytes per float)
        bb.order(ByteOrder.nativeOrder())
        vertexBuffer = bb.asFloatBuffer()
        vertexBuffer.put(FlatSurfaceVertexes)
        vertexBuffer.position(0)
        val ib = IntBuffer.allocate(FlatSurfaceIndexes.size)
        indexBuffer = ib
        indexBuffer.put(FlatSurfaceIndexes)
        indexBuffer.position(0)
        val cb = ByteBuffer.allocateDirect(FlatSurfaceColors.size * 4)
        cb.order(ByteOrder.nativeOrder())
        colorBuffer = cb.asFloatBuffer()
        colorBuffer.put(FlatSurfaceColors)
        ...
        //---------
        mTextureImageHandle = loadTextureFromResource(R.drawable.robot, context)
//        mTextureImageHandle = loadTextureFromFile()
    ...
    companion object {
        ...
        private val FlatSurfaceVertexes = floatArrayOf(
            -1f, -1f, -0f,
            1f, -1f, -0f,
            1f, 1f, -0f,
            -1f, 1f, -0f
        )
        private val FlatSurfaceColors = floatArrayOf(
            0f, 0f, 1f, 1f,
            0f, 0f, 1f, 1f,
            0f, 0f, 1f, 1f,
            0f, 0f, 1f, 1f,
        )
        private val FlatSurfaceIndexes = intArrayOf(0, 1, 2, 0, 2, 3)

        private val FlatSurfaceNormals = floatArrayOf(
            0f, 0f, 1f,
            0f, 0f, 1f,
            0f, 0f, 1f,
            0f, 0f, 1f,
