...
class ReflectionEffect(pHeight: Int, pWidth: Int) {
    ...
    init {
        ...
        // reflection effect
        mHeightHandle = GLES32.glGetUniformLocation(program, "uHeight")
        mFactorHandle = GLES32.glGetUniformLocation(program, "uFactor")
        checkGlError("Reflection get")
...
//        width = pWidth / 2
    ...
    fun draw(mvpMatrix: FloatArray) {
        ...
        // reflection effect
        GLES32.glUniform1f(mHeightHandle, height.toFloat())
        GLES32.glUniform1f(mFactorHandle, 0.3f)
        checkGlError("Reflection set")
    ...
    companion object {
        ...
        private const val FRAGMENT_SHADER_CODE =
            "precision lowp float;" +
                    ...
                    "void main() {" +
                    // reflection effect
                    "   float yPos = gl_FragCoord.y / uHeight + uFactor;" +
                    "   float dimFactor = 1.0 - 2.0 * exp(-6.0 * yPos * yPos);" +
...
//                    "   else gl_FragColor = vec4(fragmentColor.rgb, fragmentColor.a);" +
                    // reflection effect
                    "   else gl_FragColor = vec4(vec3(fragmentColor.rgb * dimFactor), fragmentColor.a);" +
                    "}"
